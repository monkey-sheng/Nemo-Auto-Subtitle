...
def infer(flowtron_path, waveglow_path, output_dir, text, speaker_id, n_frames,
    ...
    print("got text", trainset.get_text(text))
    # TODO: also allow input text to be phonemes i.e. in curly braces, get_text shouldn't arpabet parse the input text
    ...
    with torch.no_grad():
        ...
        audio = waveglow.infer(mels, sigma=0.8)
        audio = denoiser(audio, 0.03).squeeze()
