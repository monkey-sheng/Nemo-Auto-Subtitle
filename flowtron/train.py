...
def prepare_dataloaders(data_config, n_gpus, batch_size):
    ...
    valset = trainset
    valset.wav_txt_segments = valset.wav_txt_segments[100:200]
...
def load_checkpoint(checkpoint_path, model, optimizer, ignore_layers=[]):
    ...
    iteration = 0###checkpoint_dict['iteration']
    # model_dict = checkpoint_dict['model'].state_dict()
    ...
    if len(ignore_layers) > 0:
        ...
        print("ckpt dict",checkpoint_dict.keys())
...
def train(n_gpus, rank, output_directory, epochs, optim_algo, learning_rate,
    ...
    if checkpoint_path != "":
        ...
        model, _, iteration = load_checkpoint(checkpoint_path, model, optimizer, ignore_layers)
